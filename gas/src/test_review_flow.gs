...
const TEST_REVIEW_URL_      = 'https://review.rakuten.co.jp/item/1/240364_test/step-samurai-test/'; // 実行ごとに runId を付加
...
function testReviewMailFlow() {
  ...
  const results  = [];
  // 実行ごとに一意な ID。reviews の review_id と sends の type に付け、何度でも再実行できるようにする
  const runId     = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyyMMddHHmmss');
  const reviewUrl = `${TEST_REVIEW_URL_}${runId}/`;
  ...
  const csv = [
    ...
    `商品レビュー,テスト商品,${reviewUrl},5,${now},検証,"ステップ侍 検証用レビュー（自動テスト）",0,${TEST_ORDER_NUMBER_},`,
  ].join('\r\n') + '\r\n';
  step('1 CSV preview（列判定・書き込みなし）', () => {
    ...
    assert(r.sample[0].review_id === reviewUrl, `review_id はURL由来のはず: ${r.sample[0].review_id}`);
    return r.column_map;
  });
  step('2 CSV 本取込（reviews upsert + review_linked）', () => {
    ...
    assert(r.inserted === 1, `新規挿入されていない（前回の検証レビューが残っている？）: ${JSON.stringify(r)}`);
  ...
  step('5 お礼メール 実送信 → 自分のマスクアドレス', () => {
    const r = sendReviewThanksMail(tenantId, order, { recordType: `review_thanks_test_${runId}` });
  ...
  step('6 クーポンメール 実送信（ダミークーポン・API発行なし）→ 自分のマスクアドレス', () => {
    ...
    const recordType = `coupon_test_${runId}`;
    sendCouponMail(tenantId, order, dummy, { recordType });
    ...
    assert(after === before + 1, `sends に ${recordType} が追記されていない（送信されていない）`);
    const last = sends.getRange(after, 1, 1, 7).getValues()[0];
    assert(String(last[3]) === recordType && String(last[6]) === 'sent', `送信結果: type=${last[3]} result=${last[6]}`);
    return { type: recordType, result: last[6] };
  });
...
function cleanupTestReviewFlow() {
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    if (String(data[i][0]).startsWith(TEST_REVIEW_URL_) || String(data[i][0]).startsWith('test_')) { rv.deleteRow(i + 1); removed++; }
  }
